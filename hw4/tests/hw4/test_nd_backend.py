...
def test_ewise_fn(fn, shape, device):
    ...
    np.testing.assert_allclose(fn(_A, _B), fn(
        A, B).numpy(), atol=1e-5, rtol=1e-5)
...
def test_scalar_fn(fn, shape, device):
    ...
    A = ndl.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(fn(_A, _B), fn(
        A, _B).numpy(), atol=1e-5, rtol=1e-5)
...
def test_log(shape, device):
    ...
    np.testing.assert_allclose(
        np.log(_A), ndl.log(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_exp(shape, device):
    ...
    A = ndl.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(
        np.exp(_A), ndl.exp(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_relu(shape, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ndl.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.maximum(
        _A, 0), ndl.relu(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_tanh(shape, device):
    ...
    A = ndl.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(
        np.tanh(_A), ndl.tanh(A).numpy(), atol=1e-5, rtol=1e-5)
...
def test_summation(shape, axes, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ndl.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.sum(_A, axes), ndl.summation(
        A, axes=axes).numpy(), atol=1e-5, rtol=1e-5)
...
def test_broadcast_to(shape, shape_to, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ndl.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.broadcast_to(_A, shape_to), ndl.broadcast_to(
        A, shape_to).numpy(), atol=1e-5, rtol=1e-5)
...
def test_reshape(shape, shape_to, device):
    _A = np.random.randn(*shape).astype(np.float32)
    A = ndl.Tensor(nd.array(_A), device=device)
    np.testing.assert_allclose(np.reshape(_A, shape_to), ndl.reshape(
        A, shape_to).numpy(), atol=1e-5, rtol=1e-5)
...
def test_transpose(shape, axes, device):
    ...
    if axes is None:
        ...
        np_axes = axes
    np.testing.assert_allclose(np.swapaxes(_A, np_axes[0], np_axes[1]), ndl.transpose(
        A, axes=axes).numpy(), atol=1e-5, rtol=1e-5)
...
def test_logsumexp(shape, axes, device):
    ...
    if axes is None:
        ...
        t_axes = axes
    np.testing.assert_allclose(torch.logsumexp(A_t, dim=t_axes).numpy(
        ), ndl.logsumexp(A, axes=axes).numpy(), atol=1e-5, rtol=1e-5)
...
TEST_GETSETITEM_PARAMS = [
    ((3, 2), (2, 1)), ((3, 3, 4), (2, np.s_[2:], np.s_[:3]))]
