...
void Compact(const AlignedArray &a, AlignedArray *out, std::vector<int32_t> shape,
   ...
   */
    /// BEGIN SOLUTION
    RealOp(&a, out, shape, strides, offset, in_mode, -1);
    /// END SOLUTION
}
...
void EwiseSetitem(const AlignedArray &a, AlignedArray *out, std::vector<int32_t> shape,
   ...
   */
    /// BEGIN SOLUTION
    RealOp(&a, out, shape, strides, offset, out_mode, -1);
    /// END SOLUTION
}
...
void ScalarSetitem(const size_t size, scalar_t val, AlignedArray *out, std::vector<int32_t> shape,
   ...
   */
    ...
    /// BEGIN SOLUTION
    RealOp(nullptr, out, shape, strides, offset, set_mode, val);
    /// END SOLUTION
}
    ...
    for (size_t i = 0; i < m; i++) {
        for (size_t j = 0; j < p; j++) {
            ...
            for (size_t k = 0; k < n; k++) {
                size_t a_idx = i * n + k;
                size_t b_idx = k * p + j;
                out->ptr[out_idx] += a.ptr[a_idx] * b.ptr[b_idx];
            }
        }
    }
    /// END SOLUTION
   ...
   */
    ...
    for (size_t i = 0; i < m / TILE; i++) {
        for (size_t j = 0; j < p / TILE; j++) {
            size_t out_idx = i * p * TILE + j * TILE * TILE;
            for (size_t k = 0; k < n / TILE; k++) {
                size_t a_idx = i * n * TILE + k * TILE * TILE;
                size_t b_idx = k * p * TILE + j * TILE * TILE;
...
void ReduceMax(const AlignedArray &a, AlignedArray *out, size_t reduce_size) {
   ...
   */
    ...
    for (size_t i = 0; i < out->size; i++) {
        size_t a_idx = i * reduce_size;
        scalar_t maxv = a.ptr[a_idx];
        for (size_t j = a_idx; j < a_idx + reduce_size; j++) {
            maxv = std::max(maxv, a.ptr[j]);
        }
        out->ptr[i] = maxv;
    }
    /// END SOLUTION
