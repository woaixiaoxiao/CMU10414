...
#define Ceil(x) (((x) + BASE_THREAD_NUM - 1) / BASE_THREAD_NUM)
...
void Compact(const CudaArray &a, CudaArray *out, std::vector<int32_t> shape,
   ...
   */
    ...
    CompactKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, out->size, VecToCuda(shape),
                                           VecToCuda(strides), offset, mode_in);
}
...
void EwiseSetitem(const CudaArray &a, CudaArray *out, std::vector<int32_t> shape,
   ...
   */
    ...
    CompactKernel<<<dim.grid, dim.block>>>(a.ptr, out->ptr, a.size, VecToCuda(shape),
                                           VecToCuda(strides), offset, mode_out);
    /// END SOLUTION
}
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray *out, std::vector<int32_t> shape,
   ...
   */
    ...
    CompactKernel<<<dim.grid, dim.block>>>(NULL, out->ptr, out->size, VecToCuda(shape),
                                           VecToCuda(strides), offset, mode_set, val);
    /// END SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t *a, scalar_t *out, size_t count, size_t reduce_size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid < count) {
...
void ReduceMax(const CudaArray &a, CudaArray *out, size_t reduce_size) {
   ...
   */
    ...
    ReduceMaxKernel<<<dims.grid, dims.block>>>(a.ptr, out->ptr, out->size, reduce_size);
    /// END SOLUTION
}

__global__ void ReduceSumKernel(const scalar_t *a, scalar_t *out, size_t count, size_t reduce_size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid < count) {
        int start = gid * reduce_size;
        out[gid] = 0;
...
void ReduceSum(const CudaArray &a, CudaArray *out, size_t reduce_size) {
   ...
   */
    /// BEGIN SOLUTION
    CudaDims dims = CudaOneDim(out->size);
    ReduceSumKernel<<<dims.grid, dims.block>>>(a.ptr, out->ptr, out->size, reduce_size);
    /// END SOLUTION
}
