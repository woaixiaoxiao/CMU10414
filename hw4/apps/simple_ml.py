...
def epoch_general_ptb(data, model, seq_len=40, loss_fn=nn.SoftmaxLoss(), opt=None,
    ...
    np.random.seed(4)
    # BEGIN YOUR SOLUTION
    loss_sum, acc_sum, num_samples = 0.0, 0.0, 0
    # 设置模式，训练或者评估
    if opt:
        model.train()
    else:
    ...
    for i in range(0, n - 1, seq_len):
        ...
        # 训练，得到输出。其中h又要作为下一轮的输入，因此将它detach
        y_hat, h = model(X, h)
        ...
        if opt:
            ...
            opt.step()
        # 计算指标
        acc_sum += ((y_hat.numpy().argmax(axis=1)) == y.numpy()).sum()
        loss_sum += loss.numpy() * y.shape[0]
        num_samples += y.shape[0]
    return acc_sum / num_samples, loss_sum / num_samples
    # END YOUR SOLUTION
...
def evaluate_ptb(model, data, seq_len=40, loss_fn=nn.SoftmaxLoss,
    ...
    return epoch_general_ptb(
        data, model, seq_len, loss_fn(), None, None, device, dtype
        )
    # END YOUR SOLUTION
